...
//    [_imageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[TYJsonData addWithFilePathStr:@"1"]];
...
- (void)addWithImageUrl:(NSString *)urlStr {
    /*
     图片是用不能这样直接使用，这里会去进行网络请求，这样会卡住主线程，加载图片还是要用异步加载。在一个就是要使用缓存机制。
     */
    NSString *url = [NSString stringWithFormat:@"http://10.10.61.218:8080/name/%@.png",urlStr];
    [_imageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[TYJsonData addWithFilePathStr:@"1"]];
}
